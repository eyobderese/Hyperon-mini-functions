...
def generate_partitions(metta, subsets, original):
    print("signal ---am called")
    subsets= parseFromExpresstion(metta,subsets,2)
    original= set(parseFromExpresstion(original,1))
...
def generete_partionReg(metta: MeTTa):
    ...
    generatePartiton = OperationAtom('gen-partition', lambda a, b: generate_partitions(metta, a, b),
                                     ['Expression', 'Expression', 'Expression'], unwrap=True)
